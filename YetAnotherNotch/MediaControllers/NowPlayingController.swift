...
final class NowPlayingController: ObservableObject, MediaControllerProtocol {
    ...
    /// Kills the `mediaremote-adapter.pl` child and tears down the stream.
    ///
    /// Idempotent, and safe to call from `deinit` or explicitly. It must be called
    /// explicitly on app exit: this object is owned by the `MusicManager` singleton, which is
    /// never deallocated, so relying on `deinit` left the perl process running after the app
    /// had gone.
    func stop() {
        streamTask?.cancel()
        streamTask = nil
