...
class MusicManager: ObservableObject {
    ...
    public func destroy() {
        ...
        // Stop before releasing: the controller may own a child process, and deinit is not
        // guaranteed to run for something this singleton holds.
    ...
    private func updateSneakPeek() {
        if isPlaying && Defaults[.enableSneakPeek] {
            if Defaults[.sneakPeekStyles] == .standard {
                coordinator.toggleSneakPeek(status: true)
            } else {
                coordinator.toggleInlineExpansion(status: true)
            }
        }
    }
